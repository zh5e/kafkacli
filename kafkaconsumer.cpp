...
bool KafkaConsumer::queryWatermarkOffsets( const std::string &topic, int partition,
...
bool KafkaConsumer::messageAtOffset( const std::string &topic, int partition,
                                     int64_t offset, std::string &message, std::string &errstr )
{
...
bool KafkaConsumer::assign( const std::initializer_list<RdKafka::TopicPartition*>
                            &partitions )
{
...
bool KafkaConsumer::consume( std::shared_ptr<RdKafka::Message> &msgptr )
{
    msgptr.reset( consumerPtr()->consume( 3000 ) );

    if ( msgptr->err() != RdKafka::ERR_NO_ERROR ) {
        // 消费超时
        if ( msgptr->err() == RdKafka::ERR__TIMED_OUT ) {
            ILOG << "consumed  err: " << msgptr->err()
                 << ", errstr: " << QString::fromStdString( msgptr->errstr() );
            return true;
        }
        // 读到队尾
        else if ( msgptr->err() == RdKafka::ERR__PARTITION_EOF ) {
            ILOG << "consumed  err: " << msgptr->err()
                 << ", errstr: " << QString::fromStdString( msgptr->errstr() )
